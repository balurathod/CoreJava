...
public String compressForReqRes(StringBuffer strmessg)
{
	String strmessg1=strmessg.toString();
		System.out.println("stringToCompress is: '" + strmessg1.length() + "'");
		CompressionTest test=new CompressionTest();
		byte[] bytesCompressed = test.compress(strmessg1);
		int numberOfBytes = bytesCompressed.length;
		int bytesPerLine = 16;
		...
		System.out.print("bytesCompressed is:");
		...
		for (int y = 0; y < numberOfLines; y++)
		{
			...
			//System.out.print("bytesOnThisLine ::::::::"+bytesOnThisLine);
			for (int x = 0; x < bytesOnThisLine; x++)
			{
				...
				byte byteCurrent = bytesCompressed[byteIndex];

				byteAsString = Integer.toHexString(byteCurrent & 0xFF);
				if (byteAsString.length() < 2)
				{
					byteAsString = "0" + byteAsString;
				}

				byteAsString = byteAsString+"";
				//System.out.print("byteAsString ::::::::"+byteAsString);
				finalstring=finalstring+byteAsString;
			}	
			byteAsString="";
			finalstring=finalstring+"\n";
		}
		System.out.print("finalstring:::::::::"+finalstring.length());
		System.out.println();
		return finalstring;
}
}
...
/*public static void main(String[] args) {
		...
		CompressionTest1 test=new CompressionTest1();
		String testvalue= test.compressForReqRes(txtString);
